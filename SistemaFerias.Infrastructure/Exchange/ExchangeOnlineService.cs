...
{
    public string AutoReplyState { get; set; } = "Disabled"; // Disabled, Enabled, Scheduled
    public string ExternalAudience { get; set; } = "None"; // None, Known, All
...
public class ExchangeOnlineService : IExchangeOnlineService
{
    ...
    public async Task<AutoReplyConfiguration> GetAutoReplyConfigurationAsync(string email)
    {
        ...
        _logger.LogInformation("Buscando configuracao de AutoReply real via Exchange Online para: {Email}", email);
...
";
        ...
        catch (JsonException ex)
        {
            ...
            throw new InvalidOperationException("A saida do PowerShell nao estava em um formato JSON valido.", ex);
        }
    }
    ...
    public async Task<bool> DisableAutoReplyAsync(string email)
    {
        ...
        _logger.LogInformation("Desabilitando AutoReply real via Exchange Online para: {Email}", email);
