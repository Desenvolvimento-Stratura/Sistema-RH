...
public class PowerShellExecutor : IPowerShellExecutor
{
    ...
    public async Task<(bool Success, string Output, string Error)> ExecuteCommandAsync(string command)
    {
        ...
        try
        {
            ...
            if (!success)
                _logger.LogError("Erro ao executar PowerShell. ExitCode: {ExitCode}. Erro: {Error}", process.ExitCode, error);
            else
        ...
        finally
        {
            ...
            {
                _logger.LogWarning(ex, "Não foi possível remover o script temporario {ScriptPath}.", scriptPath);
            }
        }
    }
}